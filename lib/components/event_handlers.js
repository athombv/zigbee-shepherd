...
handlers.devIncoming = function (devInfo, resolve) {
	...
	var processDev = Q.fcall(function () {
		...
		try {
			...
			// Loop all endpoints to find genBasic cluster
			var basicEndpoint = 0;
			for (var i in dev.endpoints) {
				var endpoint = dev.endpoints[i];
				var clusters = endpoint.getClusterList();
				if (Array.isArray(clusters) && clusters.indexOf(0) > -1) {

					// genBasic found
					basicEndpoint = endpoint.epId;
					break;
				}
			}

			// Get basic endpoint if possible
			var basicEndpointInstance = dev.getEndpoint(basicEndpoint);
			if (!basicEndpointInstance || basicEndpointInstance instanceof Error) return dev;

			// Get manufName, modelId and powerSource information
			return self.af.zclFoundation(basicEndpointInstance, basicEndpointInstance, 0, 'read', [{ attrId: 4 }, { attrId: 5 }, { attrId: 7 }])
				.then(function (readStatusRecsRsp) {
					if (readStatusRecsRsp && Array.isArray(readStatusRecsRsp.payload)) {
						var data = {};
						readStatusRecsRsp.payload.forEach(function(item){
							if (item && item.hasOwnProperty('attrId') && item.hasOwnProperty('attrData')) {
								if (item.attrId === 7) data[attrMap[item.attrId]] = powerSourceMap[item.attrData];
								else data[attrMap[item.attrId]] = item.attrData;
							}
						});
					}
					...
					debug.request('identified device (manufacturer:', data.manufName + ', product:', data.modelId +')');

					// Save device
					return Q.ninvoke(self._devbox, 'sync', dev._getId()).then(function () {
						return dev;
					}).fail(function(err) {
						debug.shepherd('Error: failed to sync device to devbox', err);
						return dev;
					});
				})
				.catch(function(){
		...
		var numberOfEndpoints = Object.keys(dev.endpoints).length;
		...
		if (accepted) {
			Q.ninvoke(self._devbox, 'sync', dev._getId())
				.fail(function(err) {
					debug.shepherd('Error: failed to sync device to devbox (', dev.getIeeeAddr(), ')', err);
				});
			...
			debug.shepherd('Device: %s was not accepted to the network.', dev.getIeeeAddr());
			self.remove(dev.getIeeeAddr(), { reJoin: false }).then(function () {
				Q.ninvoke(self._devbox, 'remove', dev._getId());
			}).fail(function() {
				debug.shepherd('Device: %s could not be removed from devbox.', dev.getIeeeAddr());
			});
		}
	}).fail(function (err) {
		self.emit('error', err);
