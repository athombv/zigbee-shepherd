...
var Q = require('q'),
	...
	debug = {
		responseError: require('debug')('zigbee-shepherd:response-error'),
	};
...
af.send = function (srcEp, dstEp, cId, rawPayload, opt, callback) {
    ...
    areq.register(afEventCnf, deferred, function (cnf) {
        ...
        if (cnf.status === 0 || cnf.status === 'SUCCESS')   // success
	        ...
	        debug.responseError('Error: 205: no network route');
	        ...
	        debug.responseError('Error: 233: MAC no ACK');
	        ...
	        debug.responseError('Error: 183: APS no ACK');
	        ...
	        debug.responseError('Error: 240: MAC transaction expired');
	        areq.reject(afEventCnf, new Error(errText + '240. MAC transaction expired.'));
        } else {
	        debug.responseError('Error: %s: unknown error', cnf.status);
...
function dispatchIncomingMsg(type, msg) {
    ...
    if(af.controller.getCoord() == null) return; //Coordinator has not started up yet, ignore this message
